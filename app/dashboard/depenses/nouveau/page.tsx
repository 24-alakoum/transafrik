...
import { LignesForm } from '@/components/forms/LignesForm'
...
export default function NouvelleDepensePage() {
  ...
  const methods = useForm<DepenseInput>({
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-syne font-bold text-text-primary">Nouvelle Dépense</h1>
          <p className="text-text-secondary text-sm">Enregistrez une nouvelle dépense</p>
        </div>
      </div>
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="space-y-8">
          ...
          <div className="bg-bg-card rounded-2xl p-6 border border-border-base shadow-sm space-y-4">
            <LignesForm
              title="Lignes de la dépense (Articles / Détails)"
              onTotalChange={(total: number) => {
                if (total > 0) {
                  setValue('amount_fcfa', total)
                }
              }}
            />
          </div>
