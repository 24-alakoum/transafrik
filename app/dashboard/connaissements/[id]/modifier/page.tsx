...
import { updateFullBLAction } from '../../actions'
...
export default function ModifierBLPage() {
  ...
  React.useEffect(() => {
    if (bl) {
      reset({
        ...
        notes: bl.notes || '',
        containers: bl.containers?.length > 0 ? bl.containers : [{ container_number: '', type: "20'DC", seal_number: '', cargo_description: '', weight_kg: '', status: 'en_cours' }]
      })
    }
  }, [bl, reset])
  ...
  const onSubmit = async (formData: any) => {
    ...
    try {
      const result = await updateFullBLAction(blId, formData)
      if (result.success) {
        toast.success('Connaissement modifié avec succès !')
        queryClient.invalidateQueries({ queryKey: ['bl', blId] })
        queryClient.invalidateQueries({ queryKey: ['bls'] })
        router.push(`/dashboard/connaissements/${blId}`)
      } else {
  ...
  return (
    <div className="space-y-6 max-w-4xl min-w-0">
      <div className="flex items-start gap-4 min-w-0">
        <Link href={`/dashboard/connaissements/${blId}`}>
        ...
        <div className="min-w-0">
          <h1 className="text-2xl font-syne font-bold text-text-primary">Modifier Connaissement</h1>
          <p className="text-text-secondary mt-0.5">Mise à jour complète du B/L {bl.reference}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex gap-3 justify-end">
          <Link href={`/dashboard/connaissements/${blId}`}><Button type="button" variant="outline">Annuler</Button></Link>
          <Button type="submit" isLoading={isLoadingForm}>
            <Ship className="w-4 h-4 mr-2" /> Enregistrer les modifications
          </Button>
        </div>
      </form>
    </div>
