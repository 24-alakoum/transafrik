...
import { useBons } from '@/lib/queries/hooks'
...
export default function BonsLivraisonPage() {
  const { data, isLoading, isFetching } = useBons()
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary mt-1">
            Générez et envoyez vos factures et bons de livraison.
            {isFetching && !isLoading && (
              <span className="ml-2 text-accent text-xs">Actualisation...</span>
            )}
          </p>
        </div>
        <Link href="/dashboard/bons/nouveau">
          <Button>
