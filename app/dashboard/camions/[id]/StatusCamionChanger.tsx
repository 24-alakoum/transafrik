...
import { updateStatusCamionAction } from '../actions'
...
const STATUS_COLORS: Record<string, string> = {
  ...
  in_transit: 'bg-accent/10 text-accent border-accent/30 hover:bg-accent/20',
  loading: 'bg-warning/10 text-warning border-warning/30 hover:bg-warning/20',
  maintenance: 'bg-warning/10 text-warning border-warning/30 hover:bg-warning/20',
  inactive: 'bg-danger/10 text-danger border-danger/30 hover:bg-danger/20',
}

export function StatusCamionChanger({
  camionId,
  currentStatus,
}: {
  camionId: string
  ...
  const handleChange = async (newStatus: string) => {
    ...
    try {
      const result = await updateStatusCamionAction(camionId, newStatus)
      if (result.success) {
  ...
  return (
    <div className="w-full border-t border-border-base mt-6 pt-6 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        {Object.entries(TRUCK_STATUSES).map(([key, info]) => {
          ...
          return (
            <button
              ...
              className={`
                ...
                ${isActive
                  ? `${STATUS_COLORS[key] || 'bg-accent/10 text-accent border-accent/30'} border-current ring-1 ring-current/30`
